...
############新添加的############
 #   SCNN_model = SCNN(c_in=X_train.shape[1], c_out=nb_classes, nf=args.ch_size, depth=args.depth, kernel=args.k_size, pad_zero=args.pad_zero)
  #  model = SCNN_with_CA(SCNN_model)
    model = SCNN_with_CA(c_in=X_train.shape[1], c_out=nb_classes, nf=args.ch_size, depth=args.depth, kernel=args.k_size, pad_zero=args.pad_zero)

####################新添加的##########


#原来的    model = SCNN(c_in=X_train.shape[1], c_out=nb_classes, nf=args.ch_size, depth=args.depth, kernel=args.k_size, pad_zero=args.pad_zero)
    model.cuda()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if train_loss >= output:
            print('Saving model')
            save_path = 'models_save_app/DSN_sort_%s_%s_%s_%s.pth'% (args.data, args.density, args.c_size, random_str)
            train_loss = output
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pad_zero', type=str2bool, default=False, help='padding method (default: False)') 
    ##set True for UCR2018
